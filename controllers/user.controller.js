...
export const saveTestResult = async (req, res) => {
  try {
    ...
    if (!userId || !questions || questions.length === 0) {
      return res
        .status(400)
        .json({ success: false, message: "Invalid test data." });
    }
    ...
    user.testHistory.push({
      score: totalScore,
      questionsAttempted: questionAttempts,
    });
    await user.save();
    ...
    res
      .status(500)
      .json({ success: false, message: "Error saving test results." });
  }
};
...
export const getTestHistory = async (req, res) => {
  try {
    const user = await User.findById(req.params.userId).populate(
      "testHistory.questionsAttempted.questionId"
    );
